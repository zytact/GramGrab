...
// Wrapped in a block so re-pasting in DevTools doesn't trip "redeclaration of const".
{
  ...
  const STORY_USERNAME = 'antaraa_c'; // someone with an ACTIVE story right now
  const AVATAR_USERNAME = '_surojita.d_'; // any username; endpoint may return empty user
  ...
  const POST_IMAGE = 'CLolzpDDvsKsHx7Va0et9TYUSbL5kqOKoahmwE0'; // single-image post URL or shortcode
  const POST_VIDEO = 'DTriwtBEsgUGtjY0-yVkrCWvMTE2cQqfDya3aE0'; // single-video reel URL or shortcode
  const POST_SIDECAR = 'DaQQaA8iN3a'; // mixed image/video sidecar URL or shortcode
