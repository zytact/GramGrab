...
> **Current phase:** Phase 6 complete — bounded concurrency (`Effect.forEach({ concurrency: 3 })`) + partial-success accumulation for batch downloads; `graphqlFetch` retries transient failures (429 → `RateLimited`, 5xx → `GraphQLRequestFailed`, `NetworkError`) with exponential backoff (200ms base, 3 retries); 91 tests green.
> **Next step:** Phase 7 — `captureFrameFromVideo` timeout (`Effect.timeout` on `loadedmetadata`/`seeked`); optional `BrowserRuntimeService`.
