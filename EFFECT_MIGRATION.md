...
> **Current phase:** Phase 4 complete — `WebProfileInfoResponseSchema` in `src/effect/schemas.ts`; `fetchWebProfileInfoUser` Effect in `src/effect/instagram.ts`; `resolveUsernameToId` + `fetchProfilePicture` + `normalizeProfilePicture` migrated; 72 tests green.
> **Next step:** Phase 5 — Migrate one full workflow end-to-end (post/reel): shortcode GraphQL Schema, `graphqlFetch` Effect, `normalizeShortcodeMedia` Schema decode, dedupe `handleFetchMedia`/`executeDownload`.
