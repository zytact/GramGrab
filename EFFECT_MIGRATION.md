...
> **Current phase:** Phase 7 complete — `captureFrameFromVideo` migrated to `captureFrameFromVideoEffect` using `Effect.async` + `Effect.timeoutFail` (5s) on `loadedmetadata`/`seeked`; listener cleanup via `AbortSignal`; 102 tests green.
> **Next step:** Migration complete. `BrowserRuntimeService` deferred (optional; popup stays plain TS).
...
- `src/lib/browser.ts` — the shim internals. Will eventually grow an Effect layer
  _on top of_ it, not replacing the Proxy mechanism.
...
- This is behavior _enrichment_, not a behavior-preserving migration — needs
...
4. **Status → error mapping?** `graphqlFetch` today collapses all `!ok` into
   ...
   _enrichment_ (Phase 3+). Confirm this is acceptable.
...
| 2026-05-22 | Phase 6 (partial) | `src/background.ts:494–620`; `src/background.test.ts` DOWNLOAD+DOWNLOAD_MEDIA tests | `downloadMediaEffect` rewritten with `Effect.forEach({ concurrency: 3 })` + `Effect.either` per item; success type changed to `{ items, failures }`; `BrowserDownloadFailed` removed from error channel; `handleDownload` returns optional `failures[]` (all-fail → `error: 'All downloads failed'`); `handleDownloadMedia` migrated from `try/catch` loop to same `Effect.forEach` pattern; `DOWNLOAD_CONCURRENCY = 3` constant; 7 new tests (mixed/all-fail/concurrency-cap for both handlers); existing fail-fast test updated | `src/background.ts`, `src/background.test.ts`, `EFFECT_MIGRATION.md` | typecheck ✅ lint ✅ test ✅ (88/88) | none | Phase 6 retry slice: map 429 → `RateLimited`; `Effect.retry` exponential backoff on `GraphQLRequestFailed\|RateLimited` |
| 2026-05-22 | Phase 6 retry | `src/effect/instagram.ts`; `src/effect/instagram.test.ts` | 429 → `RateLimited` in `graphqlFetch`; `GRAPHQL_RETRY_SCHEDULE` = `Schedule.exponential('200 millis') ∘ Schedule.recurs(3)` applied via `Effect.retry({ schedule, while })` on `NetworkError\|GraphQLRequestFailed\|RateLimited`; `RateLimited` added to error unions of `resolveMediaEffect`+`downloadMediaEffect`; existing 429 test updated (was `GraphQLRequestFailed`, now `RateLimited`); 4 new retry tests (RateLimited exhaustion, 429 retry-then-succeed, 500 retry-then-succeed; all use `vi.useFakeTimers` + `vi.runAllTimersAsync`) | `src/effect/instagram.ts`, `src/effect/instagram.test.ts`, `src/background.ts`, `EFFECT_MIGRATION.md` | typecheck ✅ lint ✅ test ✅ (91/91) | none | Phase 7: `captureFrameFromVideo` timeout; optional `BrowserRuntimeService` |
| 2026-05-23 | Phase 7 | `src/popup.tsx:147–186`; `src/effect/` dir | `captureFrameFromVideoEffect` added to `src/effect/frame-extraction.ts` using `Effect.async((resume, signal) => …)` with `AbortSignal`-based listener cleanup + `Effect.timeoutFail({ duration: '5 seconds' })`; both `loadedmetadata` and `seeked` waits timeout-guarded; `captureFrameFromVideo` in popup replaced by thin `Effect.runPromise` caller with `catchTag` → `Error(e.reason)`; `'timeout'` arm added to `handleExportFrame` error switch; 7 unit tests in `frame-extraction.test.ts`; `BrowserRuntimeService` deferred | `src/effect/frame-extraction.ts`, `src/effect/frame-extraction.test.ts`, `src/popup.tsx`, `EFFECT_MIGRATION.md` | typecheck ✅ lint ✅ test ✅ (102/102) | none | Migration complete |
