...
| Page-owned media streaming                                          | `whatsapp/controller-runtime.test.ts`: controller fetches the selected `blob:` or same-origin photo/video source with same-origin credentials and emits metadata before bytes               | Covered.                                                                                            |
...
to a page-owned `blob:` or same-origin source, actual `activeTab` injection and frame-0 port behavior, and parity between
