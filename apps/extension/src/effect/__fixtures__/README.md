...
| `shortcode-rest-video.json` | `api/v1/media/{id}/info/` (video reel)                            | `RestShortcodeResponseSchema` - captured REST video shape with only structural media fields and replacement URLs |
...
4. Move exactly the thirteen downloaded JSON files into `.local/raw-fixtures/`. Do not edit, publish, or
...
7. Run `vp run sanitize:ig-fixtures -- --write` to transactionally replace all thirteen committed JSON
...
when they are no longer needed. The write operation installs all thirteen JSON files or rolls the
